...
```{r}
prepare_data_for_landscapergolem <- function(data, document_var, date_var, text_var, clean_text_var, colour_var, x_var, y_var,permalink_var){
...
data <- prepare_data_for_landscapergolem(data, document_var =  document, date_var = date, text_var = text, clean_text_var = clean_text, colour_var = topic, x_var = V1, y_var = V2, permalink_var = permalink)

```


Running app from another package:
1. library(LandscapeRGolem)
2. library(callr)
3. callr::r_bg(run_app, `host = "127.0.0.1"`, `port = 5000`)
...
```{r}
find_free_port <- function(start_port = 3000, end_port = 8000){
  for(port in start_port:end_port){
    # Try to make a socket connection
    socket_conn <- try(socketConnection("localhost", port = port, server = TRUE, open = "a+"), silent = TRUE)

    # Check if the connection was successful
    if(!inherits(socket_conn, "try-error")){
      # Connection was successful, close it and return the port
      close(socket_conn)
      return(port)
    }
  }
